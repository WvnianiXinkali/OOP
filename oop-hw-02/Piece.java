...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		// YOUR CODE HERE
		TPoint[] rotatedBdy = new TPoint[body.length];
		for (int i = 0; i < body.length; i++) {
			TPoint nextPoint = new TPoint(height - body[i].y - 1, body[i].x);
			rotatedBdy[i] = nextPoint;
		}
		Piece rotPiece = new Piece(rotatedBdy);
		return rotPiece;
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		for(int i = 0; i < other.body.length; i++){
			if(!Arrays.asList(body).contains(other.body[i])){
				return false;
			}
		}
		return true;
